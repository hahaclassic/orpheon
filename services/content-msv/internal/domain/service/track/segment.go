...
func NewTrackSegmentService(repo TrackSegmentRepository) *TrackSegmentService {
	return &TrackSegmentService{repo: repo}
}

func (s *TrackSegmentService) GetSegments(ctx context.Context, trackID uuid.UUID) ([]*entity.Segment, error) {
	if trackID == uuid.Nil {
...
func (s *TrackSegmentService) CreateSegments(ctx context.Context, trackID uuid.UUID, trackDuration int) error {
	if trackID == uuid.Nil {
...
func (s *TrackSegmentService) DeleteSegments(ctx context.Context, trackID uuid.UUID) error {
	if trackID == uuid.Nil {
		return ErrInvalidTrackID
	}
...
func (s *TrackSegmentService) IncrementTotalStreams(ctx context.Context, trackID uuid.UUID, segmentsIdxs []int) error {
	if trackID == uuid.Nil {
		return ErrInvalidTrackID
	}
