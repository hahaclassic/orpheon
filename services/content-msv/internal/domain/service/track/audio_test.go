...
import (
	...
	track "github.com/hahaclassic/orpheon/services/content-msv/internal/domain/service/track"
...
type AudioFileServiceSuite struct {
	suite.Suite
	service   *track.AudioFileService
...
func (s *AudioFileServiceSuite) SetupTest() {
	...
	s.service = track.NewAudioFileService(s.repo, s.converter)
...
func (s *AudioFileServiceSuite) TestGetAudioChunkInvalidParams() {
	...
	res, err := s.service.GetAudioChunk(s.ctx, chunk)
	assert.ErrorIs(s.T(), err, track.ErrInvalidChunkParams)
	assert.Nil(s.T(), res)
}
