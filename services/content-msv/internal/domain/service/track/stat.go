...
import (
	...
	usecase "github.com/hahaclassic/orpheon/services/content-msv/internal/domain/usecase/track"
)

const (
	MinSeconds           = 15 // the minimum number of listening seconds to count
	MaxDiffForSmallTrack = 2  // the minimum difference between the total duration of the segments and the total duration of the listening event to count as a small track
)
...
func (s *ListeningStatService) UpdateStat(ctx context.Context, event *entity.ListeningEvent) (err error) {
	...
	if totalDuration >= MinSeconds || diffTotal < MaxDiffForSmallTrack {
